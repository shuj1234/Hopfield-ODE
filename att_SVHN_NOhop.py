...
parser.add_argument('--save', type=str, default='./att_SVHN_NOHOP') 
...
if __name__ == '__main__':
    ...
    args.save = args.save + "_" + str(args.tau) +"_sec_" +"_run"+ str(args.run)
    ...
    feature_layers = [ODEBlock_t(ODEfunc(NC), reif_time)] 
    
    norm_layer_before_reif = [norm(NC)]
    
    reification_layers = feature_layers = [ODEBlock_t(ODEfunc(NC), reif_time)] # 6 for mnist, 7 for cifar10 ###
    # 6 is correct for default setting, which is data_aug = TRUE. 
    
    fc_layers = [norm(NC), nn.ReLU(), nn.AdaptiveAvgPool2d((1, 1)), Flatten(), nn.Linear(NC, 10)]
    ...
    model = nn.Sequential(*downsampling_layers, *feature_layers, *norm_layer_before_reif,  *reification_layers, *fc_layers).to(device)  
        ...
        if itr %( batches_per_epoch) == 0:
            with torch.no_grad():  
                ...
                logger.info(
                    ...
                    "sumloss {:.4f}| vloss {:.4f} | vtloss {:.4f} | vsumloss {:.4f}".format(
